                    ...
                    VStack(alignment: .leading, spacing: 16) {
                        ForEach(Degree.allCases, id: \.self) { degree in
                            Section(header: Text("\(degree.rawValue.capitalized) Degree")
                                        .font(.headline)
                                        .bold()){
                                            ForEach(viewModel.specializations.filter({ $0.degree == degree && $0.faculty == .kma})) { specialization in
...
struct School6View_Previews: PreviewProvider {
